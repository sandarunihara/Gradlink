<div id="sidebar" class="sidebar ">
    <div class="logodiv">
        <img src="<?php echo ROOT ?>/assets/img/grad.png" height="200" width="200" class="logo" />
    </div>
    <div id="sidebaroption" class="sidebaroption ">
        <a class="option dash" href="<?=ROOT?>/Student/StudentDash/dashboard" >
            <i class="fas option-i fa-home"></i>
            <div>
                <p>Dashboard</p>
            </div>
        </a>
        <a class="option" href="<?=ROOT?>/Student/StudentProfile/profile">
            <i class="fas fa-user"></i>
            <div>
                <p>Profile</p>
            </div>
        </a>
        <a class="option" href="<?=ROOT?>/Student/StudentAd/advertisement">
            <i class="fas fa-bullhorn"></i>
            <div>
                <p>Advertisements</p>
            </div>
        </a>
        <a class="option" href="<?=ROOT?>/Student/StudentComplaint/complaint">
            <i class="fas fa-exclamation-circle"></i>
            <div>
                <p>Complaint</p>
            </div>
        </a>
        <a class="option" href="<?=ROOT?>/Student/StudentProgress/progressReport">
            <i class="fas fa-chart-line"></i>
            <div>
                <p>Progress Report</p>
            </div>
        </a>
        <a class="option" href="<?=ROOT?>/Student/StudentTT/techtalk">
            <i class="fas fa-comments"></i>
            <div>
                <p>Tech Talk</p>
            </div>
        </a>
        <a class="option logout" href="<?=ROOT?>/logout">
            <i class="fas fa-sign-out-alt"></i>
            <div>
                <p>Log out</p>
            </div>
        </a>
    </div>
</div>
